...
const ENTITY_TYPES = [
  'opportunity', 'user', 'trust_level', 'trust_badge',
  'engagement', 'application', 'moderation', 'enforcement',
];
...
export default function AuditPage() {
  ...
  const { data: allLogs = [], isLoading } =
    searchMode === 'entity' ? entityQuery : actorQuery;
  ...
  const handleSubmit = () => { setSubmitted(true); setPage(1); };
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 space-y-4">
        ...
        <div className="flex gap-2">
          {(['entity', 'actor'] as const).map((m) => (
            <button
              key={m}
              onClick={() => { setSearchMode(m); setSubmitted(false); setPage(1); }}
        ...
        {submitted && allLogs.length > 0 && (
          <p className="text-xs text-gray-400">{allLogs.length} log entr{allLogs.length === 1 ? 'y' : 'ies'} found</p>
        )}
      </div>

      {/* Results */}
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <AuditLogTable logs={paged} isLoading={submitted && isLoading} />
